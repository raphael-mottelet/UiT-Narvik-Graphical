...
// AZERTY note: pressing physical 'Z' key maps to GLFW_KEY_W.
// So Z/S forward/back is handled here with W/S key codes.
inline void processInput(GLFWwindow* window, Camera& camera, AppState& state, float dt){
    const glm::vec3 worldUp{0.0f, 1.0f, 0.0f};

    // Forward vector in XZ from yaw
    ...
    // Conventional forward/back (W/S → AZERTY Z/S)
    ...
    // Rotations around Y (for demo/screenshot)
    const float rotSpeed = 90.0f; // deg/sec
    if(glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS) state.boxAngleY -= rotSpeed * dt; // AZERTY: Q
    if(glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS) state.boxAngleY += rotSpeed * dt;
    if(glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS) state.pyrAngleY -= rotSpeed * dt; // AZERTY: A
