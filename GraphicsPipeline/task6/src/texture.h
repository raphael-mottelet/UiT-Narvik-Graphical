...
// skip spaces and comments (# ... endline)
...
// Load a grayscale PGM (P2 or P5). Returns true on success.
inline bool loadPGM(const char* path, std::vector<unsigned char>& out, int& w, int& h){
    ...
    char m0=0, m1=0;
    fs.get(m0); fs.get(m1);
    if(!fs.good() || m0!='P' || (m1!='2' && m1!='5')) return false;
    ...
    if(m1=='2'){ // ASCII P2
        ...
        for(int j=0; j<height; ++j){
            for(int i=0; i<width; ++i){
                ...
                unsigned char g = (maxv==255) ? (unsigned char)v
                                              : (unsigned char)std::lround((double)v * 255.0 / (double)maxv);
        ...
        fs.get(); // single whitespace between header and data
        ...
        if(maxv<=255){
            ...
            if(maxv!=255){
                for(auto& px : out){
                    px = (unsigned char)std::lround((double)px * 255.0 / (double)maxv);
                }
            }
        } else {
            ...
            for(size_t i=0;i<count;++i){
                ...
                unsigned char g = (unsigned char)std::lround((double)v * 255.0 / (double)maxv);
                out[i] = g;
            }
        }
    }
...
// Tries to load a PGM image. If it fails, caller may generate fallback.
inline bool loadGrayscaleImage(const char* path, std::vector<unsigned char>& out, int& w, int& h){
    return loadPGM(path, out, w, h);
}

// Generate a simple fallback grayscale heightmap (0..255)
inline void generateFallbackHeightmap(int w, int h, std::vector<unsigned char>& out, int& ow, int& oh){
    ow = w; oh = h; out.resize(static_cast<size_t>(w) * static_cast<size_t>(h));
    for(int y=0; y<h; ++y){
        for(int x=0; x<w; ++x){
            float u = (float)x / (float)(w - 1);
            float v = (float)y / (float)(h - 1);
            float r = 0.5f + 0.5f * std::sin(10.0f * u) * std::cos(10.0f * v);
            out[y * w + x] = (unsigned char)std::round(r * 255.0f);
        }
    }
}

// Create an OpenGL single-channel (R8) texture from 8-bit grayscale pixels
