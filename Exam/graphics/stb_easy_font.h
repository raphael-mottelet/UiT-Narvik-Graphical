...
{
  static char buffer[99999]; // Holds vertices for around 500 characters
  int num_quads;
...
// Character information table: advance is horizontal spacing, h_seg and v_seg index stroke segment arrays
...
// Horizontal stroke segments for all characters, packed as small segment descriptors
...
// Vertical stroke segments for all characters, packed as small segment descriptors
...
// Per-vertex color stored as 4 unsigned bytes in RGBA order
typedef struct
{
   unsigned char c[4];
} stb_easy_font_color;

// Converts stroke segment definitions into quad vertices in an interleaved buffer used for untextured text rasterization
...
// Horizontal character spacing adjustment value added after each glyph
static float stb_easy_font_spacing_val = 0;

// Sets spacing between glyphs measured in internal font units
static void stb_easy_font_spacing(float spacing)
{
   stb_easy_font_spacing_val = spacing;
}

// Fills a vertex buffer with quad vertices for the given ASCII text string and returns the number of generated quads
...
// Returns text width in internal font units for a multi-line ASCII string
...
// Returns total height in internal font units for a multi-line ASCII string
